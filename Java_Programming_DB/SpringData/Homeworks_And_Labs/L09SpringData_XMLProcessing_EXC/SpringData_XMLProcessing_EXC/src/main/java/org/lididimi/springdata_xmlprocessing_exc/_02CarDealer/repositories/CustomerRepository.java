...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query("""
            SELECT NEW org.lididimi.springdata_xmlprocessing_exc._02CarDealer.entities.dtos.customer.CustomerWithTotalSalesDTO(c.name, COUNT(DISTINCT s.car.id), SUM(p.price ))
            FROM Customer c
            JOIN c.sales s
            JOIN s.car car
            JOIN car.parts p
